...
Speaker-aware RAG over audio: upload a podcast, get an instant summary, show notes, and a searchable transcript with speaker labels. Ask questions answered directly from the spoken content.
...
4. **Query** — intent is classified first (summary / speaker stats / RAG). For RAG queries, the question is embedded, top-k chunks are retrieved, and a speaker-labeled context prompt is streamed from a local vLLM server (Qwen3-8B).
5. **UI** — Streamlit shows an overview, auto-generated show notes (chapters + quotes + takeaways), full transcript, and an Ask tab. Source chunks link to the exact audio timestamp.
...
│   └── rag.py              # intent routing, retrieval, vLLM answer generation
├── eval/
...
├── vibevoice_repo/         # Cloned VibeVoice repo (vllm_plugin source)
